...
- (id)initWithFrame:(CGRect)frame scrollDirection:(BannerViewScrollDirection)direction images:(NSArray *)images
{
    ...
    if(self)
    {
        ...
        for (NSInteger i = 0; i < 3; i++)
        {
            ...
            if(scrollDirection == ScrollDirectionLandscape)
            {
                imgView.frame = CGRectOffset(imgView.frame, scrollView.frame.size.width * i, 0);
            }
            // 垂直滚动
            else if(scrollDirection == ScrollDirectionPortait)
            {
                imgView.frame = CGRectOffset(imgView.frame, 0, scrollView.frame.size.height * i);
            }
        ...
        //self.pageControl.currentPageIndicatorTintColor = UIColorFromRGB(0x1184d4);
